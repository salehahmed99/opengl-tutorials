...
int main() {
  ...
  GLuint resolution_loc = glGetUniformLocation(program, "u_resolution");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glUniform2fv(resolution_loc, 1, glm::value_ptr(glm::vec2(SCR_WIDTH, SCR_HEIGHT)));
...
GLuint loadShader(const std::string &path, GLenum shaderType) {
  ...
  glShaderSource(
      shader, 1, &sourceCodeStr,
      NULL); // attaches the GLSL source code string to the shader object
